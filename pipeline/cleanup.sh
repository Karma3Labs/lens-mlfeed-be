...
for batch in $(gcloud dataproc batches list --region=us-central1 --limit=10 --filter="create_time<$AGO AND state=SUCCEEDED" --format='value(name)'); 
do 
...
for batch in $(gcloud dataproc batches list --region=us-central1 --limit=10 --filter="create_time<$AGO AND state=FAILED" --format='value(name)'); 
do 
  $(gcloud dataproc batches delete --quiet $batch --region=us-central1); 
done
